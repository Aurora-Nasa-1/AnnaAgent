...
class AnnaEngineConfig(BaseModel):
    ...
    complaint_model_name: str = Field(default=anna_engine_defaults.complaint_model_name)
    counselor_model_name: str = Field(default=anna_engine_defaults.counselor_model_name)
    emotion_model_name: str = Field(default=anna_engine_defaults.emotion_model_name)
    ...
    def active_emotion_model_name(self) -> str:
        return (
            self.emotion_model_name if self.emotion_use_sft_model else self.model_name
        )
    ...
    def load(cls, root_dir: str | Path | None = None) -> "AnnaEngineConfig":
        ...
        embedding_model = (
            env("OPENAI_EMBEDDING_MODEL", None)
            or env("OPENAI_EMBEDDING_MODEL_NAME", None)
            or env("EMBEDDING_MODEL", None)
            or env("EMBEDDING_MODEL_NAME", None)
        )
        ...
        if (
            values["counselor_model_name"] == anna_engine_defaults.counselor_model_name
            and mimo_model
        ):
        ...
        if (
            values["embedding_model_name"] == anna_engine_defaults.embedding_model_name
            and embedding_model
        ):
        ...
        if (
            values["embedding_dimension"] == anna_engine_defaults.embedding_dimension
